...
for i in range(len(sentence2_list)):
    sentence2_list[(i-va_o+len(sentence2_list))%len(sentence2_list)] = f"{sentence2_list[i]}\n{ans[i]}" if ans[i] else sentence2_list[i]

for row in tsv.data:
    row['偏移后结果'] = sentence2_list  # 保持答案列不变
...
df.to_excel("m3_tsv_results_mix_negative.xlsx", index=False)
print("已保存到 m3_tsv_results_mix_negative.xlsx")